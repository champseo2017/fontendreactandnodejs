...
class UserForm extends Component {
    componentDidMount(){
        // เรียกใช้ฟังก์ชั่นในการกำหนด value ให้กับ textbox และ control ต่างๆ
        this.handleInitialize()
    }

    /* 
        กำหนดค่า value ให้กับ textbox หรือ control ต่างๆ ในฟอร์ม
        ถ้าเป็น HTML ธรรมดาก็จะำหนดเป็น value="xxx" แต่สำหรับ redux-form ต้องใช้ initialize ถ้าเป็น redux-form v.6 ต้องประกาศใช้ initialize แต่ v.7 เรียกใช้ได้เลย
    */
    handleInitialize(){
        let initData = {
            "user_type": "0",
        ...
        /* 
        ตรวจสอบก่อนว่ามี data.id หรือไม่ ถ้าไม่มีแสดงว่าเป็นการสร้างรายการใหม่ 
        ถ้ามีแสดงว่ามีการ get ข้อมูลผู้ใช้งานจึงเป็นการปรับปรุง
    */
        if(this.props.data.id){
            initData = this.props.data
            // user_type ที่รับมาเป็น init แต่ value ต้องแปลงเป็น string ก่อน
    ...
    // ฟังก์ชั่นนี้เรียกใช้ props ชื่อ onToggle จาก src/pages/User.js เพื่อปิด Modal
    toggle = () => {
        this.props.onToggle()
    }

    // ฟังก์ชั่นส่งค่าการ submit โดยส่งให้ฟังก์ชั่นชื่อ onSubmit ที่ได้จาก props
    onSubmit = (values) => {
        this.props.onSubmit(values);
    }
}

// valdate ข้อมูลก่อน submit
...
/* 
สังเกตุว่าไม่มีการใช้ connect เลยเพราะเราไม่ได้เป็นตัวจัดการ data โดยตรงแต่สิ่งที่เข้ามาเป็น props ที่ได้จาก src/pages/User.js
*/
