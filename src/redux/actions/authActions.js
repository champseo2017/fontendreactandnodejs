    ...
    return axios({
      ...
      .then(response => {
        /* 
                ...
                เก็บไว้ในคีย์ชื่อ token สาเหตุที่ต้องเก็บไว้ใน localStorage ทั้งๆ ที่เราเก็บไว้ใน store ใน reducer ก็เพราะว่าถ้าเรา refresh browser
                ค่า state ใน store ก็จะหายไปเหมือนกันครับ ดังนั้นตัว token จึงต้องหาที่เก็บไว้เพราะจะได้ไม่ต้องไปดึงจาก database ใหม่ทุกครั้ง ถ้าเป็น php ก็จะเก็บไว้ที่ cookie แต่สำหรับ workshop นี้เราเลือกเก็บไว้ใน localStorage ของ Browser
            */
